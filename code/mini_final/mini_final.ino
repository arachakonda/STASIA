...
#define ANGLE_OFFSET -2.0  // Offset angle for balance (to compensate robot own weight distribution)
...
void setup()
{
  // STEPPER PINS ON BALANCE XY STASIA STASIS SHIELD
  ...
  Serial.println("STASIA by Team Balance XY v3.0");
...
void loop()
{
  ...
  if (MPU6050_newData())
  {
    ...
    // Normal condition?
    if ((angle_adjusted < 31) && (angle_adjusted > -31))
    {
  ...
  else if (slow_loop_counter >= 100) // 1Hz
  {
    ...
    BatteryValue = (BatteryValue + stasia_readBattery(false)) / 2;
    sendBattery_counter++;
...
int stasia_readBattery(bool first_time)
{
  if (first_time)
